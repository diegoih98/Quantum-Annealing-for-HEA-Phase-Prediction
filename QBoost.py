...
lambda_val = [1,0.1,0.01,0.001,0.0001]  # Adjust as needed
...
for lambda_ in lambda_val:
    ...
    if use_hybrid_solver:
        print(f"\nUsing Leap Hybrid Solver... Lambda value={lambda_}")
        sampler = LeapHybridSampler()
        results_qa = sampler.sample(bqm, label="QBoost Hybrid Solver")
    else:
        print(f"\nUsing D-Wave Quantum Annealer with Automatic Embedding... Lambda value={lambda_}")

        qpu_sampler = DWaveSampler()
        sampler = EmbeddingComposite(qpu_sampler)

        # Compute an optimized chain strength
        #chain_strength = 2.0

        #results = sampler.sample(bqm, num_reads=1000, chain_strength=chain_strength, annealing_time=100, label="QBoost D-Wave Solver")
        results_qa = sampler.sample(bqm, num_reads=1000, annealing_time=100, label="QBoost D-Wave Solver")
    ...
    #print(f"\nLowest Energy Solution: {best_energy:.5f}")
...
# ---------------------------- Inspect Best Problem (If Applicable) ----------------------------
user_input = input("\nDo you want to inspect the best problem? (yes/no): ").strip().lower()
if user_input in ["yes", "y"] and best_bqm is not None and best_results_qa is not None:
    print("Launching D-Wave Inspector for the best problem...")
    dwave.inspector.show(best_bqm, best_results_qa)   

...
for seed in random_states:
    ...
    for lambda_ in lambda_val:
        ...
        if use_hybrid_solver:
            ...
            results_qa = sampler.sample(bqm, label=f"Hybrid Solver Round {seed+1}, λ={lambda_}")
        else:
            qpu_sampler = DWaveSampler()
            sampler = EmbeddingComposite(qpu_sampler)
            results_qa = sampler.sample(bqm, num_reads=1000, annealing_time=100, 
                                        label=f"D-Wave Solver Round {seed+1}, Lambda={lambda_}")
